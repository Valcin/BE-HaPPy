...
class FASTPT:
	...
	def one_loop_tt(self,P,P_window=None,C_window=None):
		...
		# coefficents for one_loop calculation 
		one_loop_coef=np.array([2*851/1470.,2*871/1029.,2*128/1715.,2*1/3.,2*54/35.,2*16/35.,1/3.])
	...
	def RSD_components(self,P,f, bias,P_window=None,C_window=None):
		...
		A1=bias**2*np.dot(self.A_coeff[:,0],A) + bias*f*np.dot(self.A_coeff[:,1],A) + np.dot(self.A_coeff[:,2],A)
		A3=bias**2*np.dot(self.A_coeff[:,3],A) + bias*f*np.dot(self.A_coeff[:,4],A) + np.dot(self.A_coeff[:,5],A)
		A5=bias**2*np.dot(self.A_coeff[:,6],A) + bias*f*np.dot(self.A_coeff[:,7],A) + np.dot(self.A_coeff[:,8],A)
		
		_,B=self.J_k_tensor(P,self.X_RSDB,P_window=P_window,C_window=C_window)
		
		B0=bias**2*np.dot(self.B_coeff[:,0],B) + bias*f*np.dot(self.B_coeff[:,1],B) + np.dot(self.B_coeff[:,2],B)
		B2=bias**2*np.dot(self.B_coeff[:,3],B) + bias*f*np.dot(self.B_coeff[:,4],B) + np.dot(self.B_coeff[:,5],B)
		B4=bias**2*np.dot(self.B_coeff[:,6],B) + bias*f*np.dot(self.B_coeff[:,7],B) + np.dot(self.B_coeff[:,8],B)
		B6=bias**2*np.dot(self.B_coeff[:,9],B) + bias*f*np.dot(self.B_coeff[:,10],B) + np.dot(self.B_coeff[:,11],B)
		...
		P_Ap1=bias**2*RSD_ItypeII.P_Ap1(self.k_original,P,f, bias)	
		P_Ap3=bias**2*RSD_ItypeII.P_Ap3(self.k_original,P,f, bias)	
		P_Ap5=bias**2*RSD_ItypeII.P_Ap5(self.k_original,P,f, bias)	
