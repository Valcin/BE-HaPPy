...
from matter_power_spt import P_13_reg_dd, P_13_reg_tt, P_13_reg_dt, P_13_reg_sd
...
class FASTPT:
	...
	def __init__(self,k,nu=None,to_do=None,param_mat=None,low_extrap=None,high_extrap=None,n_pad=None,verbose=False):
		...
		for entry in to_do: #convert to_do list to instructions for FAST-PT initialization
			if entry=='one_loop_dd':
				self.dd_do=True
				continue
			elif entry=='one_loop_tt':
				self.tt_do=True
				continue
			elif entry=='one_loop_dt':
				self.dt_do=True
				continue
			elif entry=='dd_bias':
				self.dd_do=True
				...
				self.dt_do=True
				self.dt_bias_do=True
				continue
			elif entry=='tt_bias':
				self.tt_do=True
				self.tt_bias_do=True
				continue
	...
	def one_loop_dd(self,P,P_window=None,C_window=None):
		...
		if (self.dd_bias_do):
			...
			sigmaPl=P_13_reg_sd(self.k_old,Ps) 
			if (self.extrap):
				...
				_, sigmaPl=self.EK.PK_original(sigmaPl)
				
			
			return P_1loop, Ps, Pd1d2, Pd2d2, Pd1s2, Pd2s2, Ps2s2, sigmaPl, sig4
	...
	def one_loop_tt(self,P,P_window=None,C_window=None):
		# routine for one-loop DM SPT calculations 	
		nu=-2
	
		# coefficents for one_loop calculation 
		one_loop_coef=np.array([2*850/1470.,2*871/1029.,2*128/1715.,2*1/3.,2*54/35.,2*16/35.,1/3.])
		...
		P22_mat=np.multiply(one_loop_coef,np.transpose(mat))
		P22=np.sum(P22_mat,1)
		P13=P_13_reg_tt(self.k_old,Ps)
		P_1loop=P22+P13 #factor to match scoccimaro expression
		...
		#~ if (self.tt_bias_do):
			...
			#~ Pd1d2=2.*(13./21*mat[0,:]+mat[4,:]+8./21*mat[1,:])
			#~ Pd2d2=2.*(mat[0,:])
			#~ Pd1s2=2.*(16./315*mat[0,:]+4./15*mat[4,:]+214./441*mat[1,:]+2./5*mat[5,:]+32./245*mat[2,:])
			#~ Pd2s2=2.*(2./3*mat[1,:])
			#~ Ps2s2=2.*(4./45*mat[0,:]+8./63*mat[1,:]+8./35*mat[2,:])
			#~ if (self.extrap):
				#~ _, Ps=self.EK.PK_original(Ps)
	...
	def one_loop_dt(self,P,P_window=None,C_window=None):
		# routine for one-loop DM SPT calculations 	
		nu=-2
	
		# coefficents for one_loop calculation 
		one_loop_coef=np.array([2*1003/1470.,2*803/1029.,2*64/1715.,2*1/3.,2*58/35.,2*12/35.,1/3.])
		...
		if (self.dd_bias_do):
			...
			Pb2t1=(13./21*mat[0,:]+mat[4,:]+8./21*mat[1,:])
			Ps2t1=(16./315*mat[0,:]+4./15*mat[4,:]+241./441*mat[1,:]+2./5*mat[5,:]+32./245*mat[2,:])
			#~ sigmaPl=P_13_reg_sd(self.k_old,Ps)
			if (self.extrap):
				_, Ps=self.EK.PK_original(Ps)
				_, P_1loop=self.EK.PK_original(P_1loop)
				_, Pb2t1=self.EK.PK_original(Pb2t1)
				_, Ps2t1=self.EK.PK_original(Ps2t1)
				#~ _, sigmaPl=self.EK.PK_original(sigmaPl)
			return P_1loop, Ps, Pb2t1, Ps2t1, sig4
	...
	def RSD_components(self,P,f, bias,P_window=None,C_window=None):
		beta = f/bias
		tabA,self.A_coeff=RSDA(f,bias)
		p_mat=tabA[:,[0,1,5,6,7,8,9]]
		self.X_RSDA=tensor_stuff(p_mat,self.N,self.m,self.eta_m,self.l,self.tau_l)	
		tabB,self.B_coeff=RSDB(f,bias)
		p_mat=tabB[:,[0,1,5,6,7,8,9]]
		self.X_RSDB=tensor_stuff(p_mat,self.N,self.m,self.eta_m,self.l,self.tau_l)	
		P_spt_dd=self.one_loop_dd(P,C_window=C_window)
		
		
		_,A=self.J_k_tensor(P,self.X_RSDA,P_window=P_window,C_window=C_window)
	   
		A1=np.dot(self.A_coeff[:,0],A) + beta*np.dot(self.A_coeff[:,1],A) + beta**2*np.dot(self.A_coeff[:,2],A)
		A3=np.dot(self.A_coeff[:,3],A) + beta*np.dot(self.A_coeff[:,4],A) + beta**2*np.dot(self.A_coeff[:,5],A)
		A5=np.dot(self.A_coeff[:,6],A) + beta*np.dot(self.A_coeff[:,7],A) + beta**2*np.dot(self.A_coeff[:,8],A)
		
		_,B=self.J_k_tensor(P_spt_dd[0],self.X_RSDB,P_window=P_window,C_window=C_window)
		
		B0=np.dot(self.B_coeff[:,0],B) + beta*np.dot(self.B_coeff[:,1],B) + beta**2*np.dot(self.B_coeff[:,2],B)
		B2=np.dot(self.B_coeff[:,3],B) + beta*np.dot(self.B_coeff[:,4],B) + beta**2*np.dot(self.B_coeff[:,5],B)
		B4=np.dot(self.B_coeff[:,6],B) + beta*np.dot(self.B_coeff[:,7],B) + beta**2*np.dot(self.B_coeff[:,8],B)
		B6=np.dot(self.B_coeff[:,9],B) + beta*np.dot(self.B_coeff[:,10],B) + beta**2*np.dot(self.B_coeff[:,11],B)
		...
		P_Ap1=RSD_ItypeII.P_Ap1(self.k_original,P,f, bias)	
		P_Ap3=RSD_ItypeII.P_Ap3(self.k_original,P,f, bias)	
		P_Ap5=RSD_ItypeII.P_Ap5(self.k_original,P,f, bias)	
	...
	def RSD_ABsum_components(self,P,f, bias,P_window=None,C_window=None):
		beta =f/bias
		A1,A3,A5, B0, B2, B4, B6, P_Ap1,P_Ap3,P_Ap5 = self.RSD_components(P,f,bias,P_window,C_window)
		ABsum_mu2 = bias * self.k_original*beta*(A1+P_Ap1) + bias**2 *(beta*self.k_original)**2 *B0
		ABsum_mu4 = bias * self.k_original*beta*(A3+P_Ap3) + bias**2 * (beta*self.k_original)**2 *B2
		ABsum_mu6 = bias * self.k_original*beta*(A5+P_Ap5) + bias**2 * (beta*self.k_original)**2 *B4
		ABsum_mu8 = bias**2 * (beta*self.k_original)**2 *B6

		return ABsum_mu2,ABsum_mu4,ABsum_mu6,ABsum_mu8

	def RSD_ABsum_mu(self,P,f,bias, mu_n,P_window=None,C_window=None):
		ABsum_mu2,ABsum_mu4,ABsum_mu6,ABsum_mu8 = self.RSD_ABsum_components(P,f,bias,P_window,C_window)
		ABsum = ABsum_mu2*mu_n**2 + ABsum_mu4*mu_n**4 + ABsum_mu6*mu_n**6 + ABsum_mu8*mu_n**8
		return ABsum

	def RSD_ABsum_multipole(self,P,f,bias, multi,P_window=None,C_window=None):
		ABsum_mu2,ABsum_mu4,ABsum_mu6,ABsum_mu8 = self.RSD_ABsum_components(P,f,bias,P_window,C_window)
		if multi == 'm':
			ABsum = ABsum_mu2*1/3. + ABsum_mu4*1/5. + ABsum_mu6*1/7. + ABsum_mu8*1/9.
		if multi == 'q':
			ABsum = ABsum_mu2*2/3. + ABsum_mu4*4/7. + ABsum_mu6*20/63. + ABsum_mu8*40/99.
